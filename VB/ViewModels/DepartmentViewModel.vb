...
Namespace ViewModels
    ...
    Partial Public Class DepartmentViewModel
        ...
        Protected Overrides Sub RefreshLookUpCollections(ByVal key As Integer)
            DepartmentCoursesLookUp = CreateLookUpCollectionViewModel(Function(x) x.Courses, Function(x) x.DepartmentID, Sub(x, m) x.Department = m, key)
            DepartmentEmployeesLookUp = CreateLookUpCollectionViewModel(Function(x) x.Employees, Function(x) x.DepartmentID, Sub(x, m) x.Department = m, key)
        End Sub
        ''' <summary>
        ''' The view model for the DepartmentCourses detail collection.
        ''' </summary>
        Public Overridable Property DepartmentCoursesLookUp As CollectionViewModel(Of Course, Integer, IDepartmentContextUnitOfWork)
        ''' <summary>
        ''' The view model for the DepartmentEmployees detail collection.
        ''' </summary>
        Public Overridable Property DepartmentEmployeesLookUp As CollectionViewModel(Of Employee, Integer, IDepartmentContextUnitOfWork)
    End Class
End Namespace
